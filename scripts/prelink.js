try {
	...
	if (VERSION < 0.28) {
		console.log(
			'You project version is ' +
				VERSION +
				' which may not compatible to rn-fetch-blob 7.0+, please consider upgrade your application template to react-native 0.27+.'
		);
	...
	console.log(
		'Add Android permissions => ' + (addAndroidPermissions == 'true')
	);
	...
	function checkVersion() {
		...
		return parseFloat(
			/\d\.\d+(?=\.)/.exec(package.dependencies['react-native'])
		);
	}
} catch (err) {
	console.log(
